...
<div class="related">
	<h3><?php printf(__("Related %s"), Inflector::humanize($_details['controller'])); ?></h3>
...
<?php
		...
		foreach (${$singularVar}[$_alias] as ${$otherSingularVar}):
			...
			echo "\t\t\t\t" . $this->Form->postLink(__('Delete'), array('controller' => $_details['controller'], 'action' => 'delete', ${$otherSingularVar}[$_details['primaryKey']]), null, __('Are you sure you want to delete').' #' . ${$otherSingularVar}[$_details['primaryKey']] . '?'). "\n";
			echo "\t\t\t</td>\n";
		echo "\t\t</tr>\n";
		endforeach;
?>
