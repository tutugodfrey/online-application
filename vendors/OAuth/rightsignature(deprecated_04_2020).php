...
class RightSignature_DEPRECATED {
  public $base_url = "https://rightsignature.com";
  public $secure_base_url = "https://rightsignature.com";
  //public $oauth_callback = "http://rstest.axiapayments.com/setup/oauth_callback.php";
  ...
  function getDocumentDetails($guid) {
    $url = "/api/documents/" . $guid . ".json";
    return $this->signAndSendRequest("GET", $url);
  }
    function extendDocument($guid) {
    $url = "/api/documents/" . $guid . "/extend_expiration.json";
    $xml = "<?xml version='1.0' encoding='UTF-8'?><will_it_blend></will_it_blend>";
    return $this->signAndSendRequest("POST",$url,$xml);
  }
  // 
